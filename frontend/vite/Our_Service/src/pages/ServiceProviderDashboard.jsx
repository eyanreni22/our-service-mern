...
const ServiceProviderDashboard = () => {
  ...
  useEffect(() => {
    if (user?._id && user.role === "provider") {
      dispatch(fetchProviderBookingsThunk());
      dispatch(fetchProviderReviews(user._id));
      dispatch(fetchProviderServicesThunk());
    }
  }, [user?._id, user?.role, dispatch]);
  ...
  <Button onClick={handleAddService}>Add Service</Button>
              ...
              <div style={{ maxWidth: "800px", margin: "2rem auto" }}>
                ...
                {providerServices.length === 0 ? (
                  ...
                  <div key={service._id} style={{ border: "1px solid #ccc", padding: "1rem", marginBottom: "1rem", borderRadius: "0.5rem" }}>
                    <h3>{service.name}</h3>
                    <p>{service.category}</p>
                    <p>${service.price}</p>
                    {service.image && <img src={service.image} alt={service.name} style={{ width: "100%", height: "200px", objectFit: "cover", borderRadius: "0.5rem", marginTop: "0.5rem" }} />}
                    <Button style={{ backgroundColor: "#ef4444" }} onClick={() => handleDeleteService(service._id)}>Delete Service</Button>
                  </div>
                ))}
              </div>
            </motion.div>
